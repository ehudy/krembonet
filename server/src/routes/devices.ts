...
import { forgetDeviceInRules } from '../alerts/notification-store.js';
...
export async function deviceAdminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      // A rule scoped to two printers, one of which is going away, should watch
      // the one that remains rather than keep a dangling id.
      forgetDeviceInRules(id);
